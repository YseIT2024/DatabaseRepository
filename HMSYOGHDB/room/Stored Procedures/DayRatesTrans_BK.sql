Create PROCEDURE [room].[DayRatesTrans_BK] 
(
	@PriceID         int,
	@ItemID          int,
	@LocationID		 int,
					...
					BEGIN
						...
						IF(EXISTS(select ItemID from [Products].[RoomDayPrice] where [ItemID] = @ItemID and [LocationID] = @LocationID and [Day] = @Day))
							BEGIN
								...
								set @Description  = @Day +' Room Rate Changed, From Rate '+ CONVERT(NVARCHAR, @OldRate , 0)+' and To Rate '+ CONVERT(NVARCHAR, @BasePrice, 0)+''
								...
								values (@ItemID,@LocationID,@Day,@BasePrice,@BasePriceSingle,@Commission,@Discount,@AddPax,@AddChild,@AddChildSr,
									...
									EXEC [reservation].[spCreateUpdateAppravalForTransaction]@ProcessTypeId, @LocationID, @UserID, @DayRatesID, 0, NULL, NULL, @ToUserId, NULL,@Description,@OldRate,@BasePrice
