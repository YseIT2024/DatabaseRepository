...
CREATE PROCEDURE [reservation].[UpdateReservationForModifications]
( 
@ReservationID INT,
@GuestID                  INT,
@TitleID                  INT,
...
AS
  BEGIN --SET XACT_ABORT ON will cause the transaction to be uncommittable
    ...
    BEGIN try
	  ...
	  Set @RoomID= (SELECT ISNULL(RoomID,0)  FROM[reservation].[ReservedRoom]  Where ReservationID=@ReservationID);
 ...
 Where [ReservationID]=@ReservationID )
          ...
          UPDATE [reservation].[reservation] SET    
				[reservationtypeid] = @ReservationTypeID,	[reservationmodeid] = @ReservationModeID,	[expectedcheckin] = @ExpectedCheckIn,
                 ...
                 [extrachildse] = @ExChildranSr          WHERE  reservationid = @ReservationID
		 ...
		 DELETE          FROM   [reservation].[reservationtaxdetails]          WHERE  reservationid = @ReservationID;
    ...
    --                              @GuestDateOfBirth,@GuestType,@CountryID,NULL,NULL,
		...
		Begin
			...
			Date=GETDATE()
			where ReservationID=@ReservationID and RoomID=@RoomID		
		  --EXEC  [reservation].[spMoveInHouse1] @NewroomId,@RoomId,@ReservationID,@ExpectedCheckIn,@ExpectedCheckOut,0.00,@TodaysDate,'change Room for Modification',1,1,76,0, @ItemID;	 
		  --SET @Message = 'Message : '+' '+cast(@NewroomId As Varchar(500))+' '+' '+' '+cast( @RoomId As Varchar(500))+' '+cast( @ReservationID As Varchar(500))+' '+cast( @ExpectedCheckIn As Varchar(500))+' '+cast( @ExpectedCheckOut As Varchar(500))+' '+cast( 0.00 As Varchar(500))+' '+cast( @TodaysDate As Varchar(500))+' '+'change Room for Modification'+' '+cast( 1 As Varchar(500))+' '+cast( 1 As Varchar(500))+' '+cast( 76 As Varchar(500))+' '+cast( 0 As Varchar(500))+' '+ 'Reservation Room Number Updated Successfully.';
