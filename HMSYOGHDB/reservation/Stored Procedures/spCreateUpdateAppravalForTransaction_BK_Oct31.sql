...
-- Description:	CREATE UPDATE APPROVAL For Transaction Screen
-- =============================================
Create PROCEDURE [reservation].[spCreateUpdateAppravalForTransaction_BK_Oct31]
(
	 @ProcesstypeId INT
	,@LocatioId INT
	,@CreatedBy INT
...
BEGIN
		...
		DECLARE @LogLevel INT;
			...
			(
				[ProcesstypeId],[LocatioId],[CreatedOn],[CreatedBy],[ApprovalDescription],[RefrenceNo],[ApprovalStatus],[ToRoleId],[ToUserId],[LogLevel],[Remark],[OldRate],[NewRate]
			)
			VALUES
			(	
				@ProcesstypeId,@LocatioId,GETDATE(),@CreatedBy,@ApprovalDescripion,@RefrenceNo,@ApprovalStatus,@ToRoleId,@ToUserId,@LogLevel,@Remark,@OldRate,@NewRate
			)
